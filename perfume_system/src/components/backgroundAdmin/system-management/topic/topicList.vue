    ...
    <div class="fillcontain">
        <div class="table_container">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>热门话题管理</el-breadcrumb-item>
                <el-breadcrumb-item>热门话题列表</el-breadcrumb-item>
            </el-breadcrumb>
            <div class="add">
...
import projcetAddOrEdit from './addTopic'
export default {
    components: {
        projcetAddOrEdit
    },
    ...
    methods: {
        ...
        handleEdit(index, row) {
            ...
            this.formObj = obj
            this.projcetAddOrEditShow = true
            this.titleText = '编辑热门话题'
        },
        handleDelete(index,row) {
            console.log(row)
            var id = row._id
            axios.posttopicDelete(id, res => {
                console.log(res)
                if (res.code == 'success') {
        ...
        submitHandle(obj, flag) {
            this.projcetAddOrEditShow = false
            if (flag === '修改') {
                axios.posttopicEditor(obj._id, obj, res => {
                    if (res.code == 'success') {
                        this.$message.success('修改热门话题成功')
                        this.formObj = obj
                        this.getData();
                    }
                })
            } else {
                axios.posttopicAdd(obj, res => {
                    if (res.code == 'success') {
                        this.$message.success('添加热门话题成功')
                        this.$router.push({
                            name: 'TopicList'
                        })
                        this.getData();
